...
- **PR #21 opened** (`feat/axe-a11y-testing`) — the above; self-review caught two consistency nits (fixed: `clipboard`'s duplicated fixture HTML hoisted into a shared `A11Y_HTML` const; `form-confirm`'s accessibility checks rescoped from `document.body` to the `widget` element, matching the other 13 controllers)
- Triaged via `copilot-review-triage` on PR #21: Copilot's one comment was valid — `test/axe.js` imported axe-core's named `{ run }` export, but axe-core ships pure CJS with no `exports` map (`"main": "axe.js"` only), so `run` only resolved because Vitest/Vite's runtime CJS interop happens to proxy every property as a named export; verified directly that plain Node ESM resolves `run` to `undefined` (and an unrelated grab-bag of internal symbols) while `import axe from "axe-core"; axe.run(...)` works in both. Fixed; 202 tests still pass, no behavior change.
...
- **PR #21** (`feat/axe-a11y-testing`) — axe-core accessibility testing across all 15 controllers; self-review nits fixed, Copilot-triaged and fixed (see Key decisions); 202 tests passing, format/lint clean. Awaiting merge.

**PR #13**, **PR #16**, **PR #17**, and **PR #19** in this repo all merged; **PR #5** (`feat/datetime-local-demo`) in `stimulus-snippets-demo` also merged.
