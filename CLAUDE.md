...
- **PR #22** (`feat/table-sort-controller`) — `table-sort` controller: click a `<th>` to sort rows, auto-detects string/number/date type from the first row, `data-table-sort-type` override (validated against an allow-list, falls back to auto-detection otherwise — see Key decisions) and `data-table-sort-value` per-cell override, `aria-sort` kept in sync; Copilot-triaged and fixed; 213 tests passing, format/lint clean. Awaiting merge.

**PR #13**, **PR #16**, **PR #17**, **PR #19**, and **PR #21** in this repo all merged; **PR #5** (`feat/datetime-local-demo`) in `stimulus-snippets-demo` also merged.
