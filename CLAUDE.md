...
- **PR #22** (`feat/table-sort-controller`) — `table-sort` controller: click a `<th>` to sort rows, auto-detects string/number/date type from the first row, `data-table-sort-type` override and `data-table-sort-value` per-cell override, `aria-sort` kept in sync; 212 tests passing, format/lint clean. Not yet opened.
